...
class Transformer(nn.Module):
    def __init__(self, configs):
        ...
        encoder_layer = nn.TransformerEncoderLayer(d_model=configs.d_model, nhead=configs.n_heads,
                                                   dim_feedforward=configs.d_ff, activation=configs.activation,
                                                   dropout=0.05)
        encoder_norm = nn.LayerNorm(configs.d_model)
        self.model = nn.TransformerEncoder(encoder_layer, 1, encoder_norm)

        self._reset_parameters()

    def forward(self, x, src_mask=None, src_key_padding_mask=None):
        return self.model(x, mask=src_mask, src_key_padding_mask=src_key_padding_mask) + x  # TODO
...
class Model(nn.Module):
    ...
    def __init__(self, configs):
        ...
        self.pred_len = configs.pred_len
        self.tran = nn.ModuleList([Transformer(configs)
    ...
    def forecast(self, x_enc, x_mark_enc, x_dec, x_mark_dec):
        ...
        for i in range(self.layer):
            ...
            enc_out = self.tran[i](self.layer_norm(self.model[i](enc_out)))
        # porject back
